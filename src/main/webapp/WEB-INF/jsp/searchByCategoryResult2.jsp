...
<div class="categoryPageDiv">
    ...
    <div class="categoryProducts">
        <c:forEach items="${lists}" var="page" >
            <p>
            </p>
            <c:forEach items="${page.listItem}" var="product">
            ...
            <div class="page">
                <a href="?pageNo=1&keyword=${keyword}" rel="rel">首页</a>
                <c:if test="${page.pageNo >1}">
                    <a href="?pageNo=${page.pageNo+1}&keyword=${keyword}" rel="rel">&lt; 上一页</a>
                </c:if>
                <c:forEach begin="1" end="${page.pageSum}" varStatus="status">
                    <a href="?pageNo=${status.count}&keyword=${keyword}" class="active" <c:if test="${page.pageNo eq status.count}"> style="font-weight: bold;" </c:if>>${status.count}</a>
                </c:forEach>
                <a href="?pageNo=${page.pageNo+1}&keyword=${keyword}" rel="rel">&; 下一页</a>
            </div>
        </c:forEach>
    </div>
    <%--清除浮动？？！！emmmmm--%>
